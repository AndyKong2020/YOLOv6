...
|---|---|
| 任务编号 | YOLOv6-0.4.0 |
| 任务用途 | COCO 目标检测推理、评估、训练 smoke 与 NPU 亲和性复现 |
...
- [x] 训练 smoke 适配:训练设备选择支持 NPU;NPU 训练先关闭 CUDA AMP/GradScaler,走 FP32;标准 YOLOv6 loss 中 `.cuda()`/`torch.cuda.empty_cache()` 改为当前加速器;`loss.preprocess()` 输出显式 `float32`,避免 label assignment 中 double tensor 触发 NPU `Min` 不支持。
- 命令:
```bash
# COCO val 精度复现
...
# 训练 smoke,使用少量 COCO val 样本构造 train/val 子集
...
## 3. 验证用例
- 输入数据:COCO val2017,共 5000 张图片;P5 输入尺寸 640,batch size 32。训练 smoke 另用 8 张 train + 4 张 val 的小子集,只验证训练链路,不代表训练精度。
- 运行命令:见第 2 节,覆盖 COCO val、FP32 speed、FP16 speed、1 epoch 训练 smoke。
...
- 实测输出:
  ...
  - 训练 smoke:最小 NPU 训练补丁后,YOLOv6-N 在物理后段空闲卡上完成 1 epoch,4/4 step,生成 `last_ckpt.pt` 与 `best_ckpt.pt`,日志显示 `Using 1 NPU for training`、`Training completed`。
  - 训练 assigner:首次 NPU 训练补丁会在 label assignment 中因 `DT_DOUBLE` 输入触发 NPU `Min` error code `161002`,随后回退 CPU;将 `loss.preprocess()` 生成的 numpy array 固定为 `float32` 后,同一 smoke 不再出现 `CPU mode for This Batch`。
...
| 指标 | 数值 |
|---|---|
| 能否在 NPU 跑通 | 推理/评估能跑通。训练原始入口不能直接跑;最小 NPU 训练补丁 + float32 assigner 修正后,YOLOv6-N 1 epoch smoke 跑通。 |
...
| 关键算子是否回退 CPU | 推理主体无关键 CPU 回退;NMS 回退 CPU;pycocotools 指标统计在 CPU。训练 smoke 在 float32 assigner 修正后未观察到 label assignment CPU fallback。 |
| 性能(吞吐/时延) | FP32 inference 0.30 ms/image;FP16 inference 0.20 ms/image;CPU NMS 在 speed 阈值下约 0.9 ms/image,在 COCO val 低阈值下 149.31 ms/image。训练 smoke 只验证可运行,不作为性能数据。 |
...
| 调度(host/head) | preprocess、CPU NMS、pycocotools、训练保存/eval | 高 | NMS 是当前端到端瓶颈;训练 smoke 可完成,但 DDP、AMP、distill/fuse_ab/QAT 未验证。 |
...
- 算子回退清单:
  ...
  - 训练 label assignment:原始 NPU 补丁下因 `DT_DOUBLE` 输入触发 NPU `Min` 不支持并回退 CPU;固定 `loss.preprocess()` 为 `float32` 后,训练 smoke 未再回退。
- profiler 摘要:
  - 未采集完整 STARS/torch_npu profiler。现有日志只提供 preprocess、inference、NMS 三段时间和训练 smoke 完成性证据。AIC/AIV/MTE、tile 驻留、NDDMA/CV fusion、训练 backward/assigner 热点均为待测。
...
| 原始训练入口 CUDA 绑定 | `select_device()` 强制 CUDA;训练 loop 使用 CUDA AMP/GradScaler/empty_cache;loss 多处 `.cuda()` | 对 NPU 训练是硬阻塞 | 建立统一 accelerator abstraction;NPU 默认 FP32 或 torch_npu AMP 路径 | 最小补丁后可跑 smoke,但不是完整训练适配 |
| 训练 assigner double tensor | `loss.preprocess()` 的 numpy array 默认为 float64,NPU `Min` 不支持 DT_DOUBLE | 对纯 NPU label assignment 是硬阻塞 | 固定 label preprocess 为 float32;后续对 assigner 做 dtype 单测 | 修正后 smoke 未再 CPU fallback |
...
| 训练分支未补齐 | 本轮只跑标准 YOLOv6-N 单卡 FP32 smoke | 非硬阻塞,但限制训练结论 | 补 AMP、DDP/HCCL、distill、fuse_ab、QAT/quant、resume 和多 epoch 稳定性测试 | 当前只声明 smoke 通过 |
| HBM/利用率未 profile | npu-smi 未采稳定利用率曲线 | 非硬阻塞 | STARS/torch_npu profiler 采 AIC/AIV/MTE、kernel timeline、HBM 峰值 | 用日志三段耗时做初判 |

## 6. 结论
- 运行方案(NPU / NPU+CPU / CPU):NPU+CPU。YOLOv6-N 推理主体在 NPU 上跑通且亲和,COCO mAP 与 README 基准对齐;NMS 与 COCO metric 当前在 CPU。训练原始入口不支持 NPU,但最小训练补丁后单卡 FP32 smoke 可完成 forward/backward/optimizer step/save/eval。
- 待办 / 风险:
  ...
  - 补 DDP/HCCL、distill、fuse_ab、QAT/quant、resume 训练分支;这些路径仍有 CUDA 绑定,不能由当前 smoke 外推。
